...
| **v2.1.0** | Students who want the reference **plus forgot / reset password** | Everything in v2.0.0 plus **Forgot / Reset Password**: self-service password reset for local accounts. Unauthenticated `/forgot-password` form sends an email with a single-use, 1-hour reset link; clicking the link lands on `/reset-password` where the user enters a new password (strength-policy enforced, bcrypt-hashed). No session written on reset; user logs in fresh with the new password. Rate-limited per-IP; CSRF-protected on reset POST. A successful reset also clears the per-account lockout counters (a reset proves account control). Sixth DB-schema change (2 columns on `users`); stdlib only, no new dependency. |
